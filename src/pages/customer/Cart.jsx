...
export default function Cart() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      // Refresh stock in case something sold out
      const ids = items.map(i => i.id)
      supabase.from('products').select('id, stock').in('id', ids).then(({ data }) => {
        if (data) refreshStock(data)
      })
  ...
  const handleRemove = (item) => {
    removeItem(item.id)
    const toastId = toast(`Removed — `, {
      action: {
        label: 'Undo',
        onClick: () => {
          clearTimeout(undoTimers.current[item.id])
          updateQty(item.id, item.quantity) // won't restore if id removed — add back
          // Re-add item
          import('../../contexts/CartContext').then(({ useCart: _ }) => {})
        }
      }
    })
    _ = toastId
    ...
    return (
      <>
        ...
        <main className="max-w-3xl mx-auto px-4 py-16 text-center">
          ...
          <Link to="/" className="bg-brand text-white px-8 py-3 rounded-lg font-medium hover:bg-brand-dark transition-colors">
            Browse products
          </Link>
        </main>
      </>
              ...
              return (
                <div key={item.id} className={`flex gap-4 p-4 bg-white rounded-xl border border-cream-200 ${item._soldOut ? 'opacity-60' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item._soldOut && (
                      <p className="text-red-600 text-xs mt-1 font-medium">This item is no longer available</p>
                    )}
                    {item._stockWarning && !item._soldOut && (
                      <p className="text-amber-600 text-xs mt-1 font-medium">Only {item.stock} left — quantity updated</p>
                    )}
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <div className="flex items-center border border-cream-200 rounded-lg overflow-hidden">
                        <button
                          ...
                          aria-label="Decrease"
                          className="px-2.5 py-1.5 hover:bg-cream-100 disabled:opacity-40 text-sm"
                        >
                          −
                        </button>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl border border-cream-200 p-6 sticky top-20">
              ...
              <form onSubmit={handleSubmit} noValidate className="space-y-4">
                ...
                <div>
                  <label htmlFor="field-note" className="block text-sm font-medium text-gray-700 mb-1">
                ...
                {placeError && (
                  <p className="text-red-600 text-sm p-3 bg-red-50 rounded-lg">{placeError}</p>
                )}

                <button
                  type="submit"
                  disabled={placing || items.some(i => i._soldOut)}
                  className="w-full bg-brand text-white py-3 rounded-lg font-semibold hover:bg-brand-dark transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
