...
import { useParams, useNavigate, Link, unstable_useBlocker as useBlocker } from 'react-router-dom'
...
export default function ProductForm() {
  ...
  useBlocker(() => {
    if (isDirty) return !window.confirm('You have unsaved changes. Leave anyway?')
    return false
  })
  ...
  const handleSubmit = async (e) => {
    ...
    if (imageFile) {
      ...
      if (uploadError) {
        toast('Image upload failed: ' + uploadError.message)
        setSaving(false)
        return
      }
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <Link to="/admin/products" className="text-sm text-brand hover:underline">← Products</Link>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <div>
          ...
          <div
            ...
            className="border-2 border-dashed border-cream-200 rounded-xl p-6 text-center cursor-pointer hover:border-brand transition-colors relative overflow-hidden"
          >
            {imagePreview ? (
        ...
        <div>
          ...
          <input
            ...
            className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand ${errors.name ? 'border-red-400' : 'border-gray-200'}`}
          />
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full border rounded-lg pl-7 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand ${errors.price ? 'border-red-400' : 'border-gray-200'}`}
              />
            </div>
          ...
          {showNewCat ? (
              ...
              <button
                ...
                onClick={() => {
                  if (newCat.trim()) {
                    ...
                    setField('category', newCat.trim())
                  }
                  setShowNewCat(false)
                  setNewCat('')
                }}
                className="px-3 py-2 bg-brand text-white text-sm rounded-lg"
              >
                Add
              </button>
            ...
            <div className="flex gap-2">
              <select
                ...
                className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand bg-white"
              >
              ...
              <button
                ...
                className="text-sm text-brand hover:underline px-2 shrink-0"
              >
                + New
              </button>
            </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`w-12 h-7 rounded-full transition-colors relative ${form.is_active ? 'bg-brand' : 'bg-gray-200'}`}
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
          <button
            type="submit"
            disabled={saving}
            className="flex-1 sm:flex-none bg-brand text-white px-8 py-3 rounded-lg font-semibold hover:bg-brand-dark transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
          >
